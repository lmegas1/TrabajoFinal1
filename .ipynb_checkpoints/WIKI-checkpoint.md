...
- **General:** Desarrollar una aplicación de IA que analice integralmente la salud mental estudiantil combinando datos individuales y contextuales, usando exclusivamente librerías de software libre.
- **Específicos:**
...
El uso de herramientas colaborativas de código abierto garantiza reproducibilidad, facilita la colaboración y elimina barreras de acceso económico. Jupyter Lab como entorno permite combinar código, visualizaciones y narrativa en un único documento interactivo, alineado con las mejores prácticas de la ciencia de datos reproducible.
...
![Figura 1 — Distribución General](assets/viz_01_distribucion.png)
...
![Figura 2 — Correlaciones](assets/viz_02_correlacion.png)
...
![Figura 3 — Carrera y Contexto](assets/viz_03_carrera_contexto.png)
...
![Figura 4 — Modelos de IA](assets/viz_04_modelos.png)
...
![Figura 5 — Clustering](assets/viz_05_clustering.png)

---

> **Nota para GitHub:** Para visualizar las imágenes en esta Wiki, sube los archivos PNG a la carpeta `assets/` del repositorio y reemplaza `TU-USUARIO/TU-REPO` en las URLs por tus datos:  
> `https://raw.githubusercontent.com/TU-USUARIO/TU-REPO/main/assets/viz_01_distribucion.png`
...
1. **La integración de dos datasets amplía el poder analítico:** El JOIN entre el CSV (salud mental) y el SQLite (contexto socioeconómico) reveló relaciones que ninguna fuente por separado podría mostrar: estudiantes de nivel socioeconómico bajo con alta carga laboral tienen el doble de riesgo de problema de salud mental que estudiantes de nivel alto.

2. **Alta prevalencia confirmada:** El 42.8% de los 5 000 estudiantes presenta al menos depresión o ansiedad, superando los promedios históricos de la literatura y evidenciando una crisis de salud mental en el ámbito universitario ecuatoriano.
...
4. **Perfil de mayor riesgo identificado:** El Perfil 4 del clustering (poco sueño, alta carga de trabajo y bajo bienestar) concentra un 61% de incidencia de problema de salud mental. Intervenir en este perfil tendría el mayor impacto institucional.
...
7. **El JOIN con Pandas es la operación clave del proyecto:** `pd.merge(on='student_id')` en una sola línea integra 32 columnas de dos fuentes heterogéneas sin pérdida de datos, demostrando el poder de compartir una ID entre datasets de distinto formato.
...
10. **20 features mejoran la clasificación:** Al incluir variables del SQLite en los modelos (nivel socioeconómico, horas de trabajo, satisfacción), la accuracy subió respecto a modelos con solo las variables del CSV, validando el valor del JOIN.

### 5.3 Reflexión Final

Este proyecto demuestra que es posible construir un pipeline completo de análisis de datos con IA — desde la carga de dos fuentes heterogéneas hasta la predicción y clustering — usando exclusivamente software libre. La decisión de compartir una ID entre el CSV y la base de datos SQLite es el elemento arquitectónico central que habilita un análisis mucho más rico y contextualizado.
...
*Dataset sintético generado con distribuciones estadísticas basadas en literatura científica. Uso exclusivamente académico.*  
